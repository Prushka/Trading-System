...
public class TradeEditor {
    ...
    public void edit(Trade currTrade, User editingUser, String... args) {
        ...
        for (UserOrderBundle user: currTrade.getOrder().getTraders()){
            if (user.getUser().getEmail().equals(editingUser.getEmail()) && !user.isTradeConfirmed() && user.getEdits() == editLimit){
                cancelOrder(currTrade);
            } else if (user.getUser().getEmail().equals(editingUser.getEmail()) && !user.isTradeConfirmed() && user.getEdits() < editLimit){
                if (args.length == 5){
                ...
                user.setTradeConfirmed(true);
            } else if (!user.getUser().equals(editingUser)){
                user.setTradeConfirmed(false); // Need to alter so if they aren't in the trade at all, this doesn't happen
            }
        }
    }
